...
#include "exi.h"
#include "gc/asm.h"
#include "gc/card_cmn.h"
#include "gc/lwp.h"
#include "gc/ogcsys.h"
#include "gc/processor.h"
#include "gc/semaphore.h"
#include "gc/system.h" //! TODO: use the sdk functions (when applicable)
#include "os.h"
#include "types.h"
#include "vc.h"
// #include "gc/card_fat.h"
...
#define CARDIO_OP_IOERR_FATAL \
    (CARDIO_OP_IOERR_PARAM | CARDIO_OP_IOERR_WRITE | CARDIO_OP_IOERR_ADDR | CARDIO_OP_IOERR_CRC | CARDIO_OP_IOERR_ILL)
...
static OSThreadQueue _ioEXILock[MAX_DRIVE];
...
static __inline__ u32 __check_response(s32 drv_no, u8 res) {
    ...
    if (_ioFlag[drv_no] == INITIALIZING && res & MMC_ERROR_IDLE) {
        ...
        if (res & MMC_ERROR_PARAM)
            _ioError[drv_no] |= CARDIO_OP_IOERR_PARAM;
        if (res & MMC_ERROR_ADDRESS)
            _ioError[drv_no] |= CARDIO_OP_IOERR_ADDR;
        if (res & MMC_ERROR_CRC)
            _ioError[drv_no] |= CARDIO_OP_IOERR_CRC;
        if (res & MMC_ERROR_ILL)
            _ioError[drv_no] |= CARDIO_OP_IOERR_ILL;
    }
...
static void __exi_wait(s32 drv_no) {
    ...
    do {
        if ((ret = EXILock(drv_no, _ioCardSelect[drv_no], __exi_unlock)) == 1)
            break;
        OSSleepThread(&_ioEXILock[drv_no]);
    } while (ret == 0);
}
...
static s32 __card_writecmd0(s32 drv_no) {
    ...
    cnt = 0;
    while (cnt < 20) {
        if (EXIImmEx(drv_no, dummy, 128, EXI_WRITE) == 0) {
            EXIDeselect(drv_no);
            EXIUnlock(drv_no);
    ...
    if (EXISelect(drv_no, _ioCardSelect[drv_no], _ioCardFreq[drv_no]) == 0) {
        EXIUnlock(drv_no);
        return CARDIO_ERROR_NOCARD;
    }
...
#endif
    if (EXIImmEx(drv_no, cmd, 5, EXI_WRITE) == 0) {
        EXIDeselect(drv_no);
        EXIUnlock(drv_no);
        return CARDIO_ERROR_IOERROR;
    }

    if (EXIImmEx(drv_no, &crc, 1, EXI_WRITE) == 0) {
        EXIDeselect(drv_no);
        EXIUnlock(drv_no);
        return CARDIO_ERROR_IOERROR;
    }
...
static s32 __card_writecmd(s32 drv_no, void* buf, s32 len) {
    ...
    if (EXIImmEx(drv_no, dummy, 10, EXI_WRITE) == 0) {
        EXIDeselect(drv_no);
        EXIUnlock(drv_no);
        return CARDIO_ERROR_IOERROR;
    }
    ...
    printf("sd command: %02x %02x %02x %02x %02x %02x\n", ((u8*)buf)[0], ((u8*)buf)[1], ((u8*)buf)[2], ((u8*)buf)[3],
           ((u8*)buf)[4], crc);
#endif
    if (EXIImmEx(drv_no, buf, len, EXI_WRITE) == 0) {
        EXIDeselect(drv_no);
        EXIUnlock(drv_no);
        return CARDIO_ERROR_IOERROR;
    }
...
static s32 __card_readresponse(s32 drv_no, void* buf, s32 len) {
    ...
    if ((_ioTransferMode[drv_no] == CARDIO_TRANSFER_DMA
             ...
             : EXISelect(drv_no, _ioCardSelect[drv_no], _ioCardFreq[drv_no])) == 0) {
        EXIUnlock(drv_no);
        return CARDIO_ERROR_NOCARD;
    }
    ...
    for (cnt = 0; cnt < 16; cnt++) {
        *ptr = 0xff;
        if (EXIImmEx(drv_no, ptr, 1, EXI_READWRITE) == 0) {
            EXIDeselect(drv_no);
            EXIUnlock(drv_no);
            return CARDIO_ERROR_IOERROR;
        }
    ...
    if (cnt >= 16)
        ret = CARDIO_ERROR_IOTIMEOUT;
    if (len > 1 && ret == CARDIO_ERROR_READY) {
        *(++ptr) = 0xff;
        if (EXIImmEx(drv_no, ptr, len - 1, EXI_READWRITE) == 0)
            ret = CARDIO_ERROR_IOERROR;
    }
...
static s32 __card_stopresponse(s32 drv_no) {
    ...
    if ((ret = __card_stopreadresponse(drv_no, _ioResponse[drv_no], 1)) != 0)
        return ret;
...
static s32 __card_dataresponse(s32 drv_no) {
    ...
    if ((ret = __card_datares(drv_no, _ioResponse[drv_no])) != 0)
        return ret;
    res = _SHIFTR(_ioResponse[drv_no][0], 1, 3);
    if (res == 0x0005)
        ret = CARDIO_OP_IOERR_CRC;
    else if (res == 0x0006)
        ret = CARDIO_OP_IOERR_WRITE;
    ...
    *ptr = 0xff;
    ...
    if (_ioTransferMode[drv_no] == CARDIO_TRANSFER_DMA) {
        ...
        for (cnt = 0; cnt < len; cnt++)
            ptr[cnt] = 0xff;
        if (EXIImmEx(drv_no, ptr, len, EXI_READWRITE) == 0) {
            EXIDeselect(drv_no);
            EXIUnlock(drv_no);
            return CARDIO_ERROR_IOERROR;
        }
    ...
    res[0] = res[1] = 0xff;
    if (EXIImmEx(drv_no, res, 2, EXI_READWRITE) == 0) {
        EXIDeselect(drv_no);
        EXIUnlock(drv_no);
        return CARDIO_ERROR_IOERROR;
    }
...
static s32 __card_datawrite(s32 drv_no, void* buf, u32 len) {
    ...
    dummy[0] = 0xfe;
    if (EXIImmEx(drv_no, dummy, 1, EXI_WRITE) == 0) {
        EXIDeselect(drv_no);
        EXIUnlock(drv_no);
        return CARDIO_ERROR_IOERROR;
    }
    ...
    ret = CARDIO_ERROR_READY;
    if (EXIImmEx(drv_no, &crc, 2, EXI_WRITE) == 0)
        ret = CARDIO_ERROR_IOERROR;
...
static s32 __card_multiwritestop(s32 drv_no) {
    ...
    dummy[0] = 0xff;
    if (EXIImmEx(drv_no, dummy, 1, EXI_READWRITE) == 0) {
        EXIDeselect(drv_no);
        EXIUnlock(drv_no);
        return CARDIO_ERROR_IOERROR;
    }
...
static s32 __card_response2(s32 drv_no) {
    ...
    if ((ret = __card_readresponse(drv_no, _ioResponse[drv_no], 2)) != 0)
        return ret;
    if (!(_ioResponse[drv_no][0] & 0x7c) && !(_ioResponse[drv_no][1] & 0x9e))
        return CARDIO_ERROR_READY;
...
static s32 __card_sendopcond(s32 drv_no) {
    ...
    do {
        ...
        if ((ret = __card_readresponse(drv_no, _ioResponse[drv_no], 1)) != 0)
            return ret;
        if ((ret = __check_response(drv_no, _ioResponse[drv_no][0])) != 0)
            return ret;
        if (!(_ioError[drv_no] & CARDIO_OP_IOERR_IDLE))
            return CARDIO_ERROR_READY;
...
static s32 __card_sendCMD8(s32 drv_no) {
    ...
    if ((ret = __card_readresponse(drv_no, _ioResponse[drv_no], 5)) != 0)
        return ret;
    ret = __check_response(drv_no, _ioResponse[drv_no][0]);
...
#endif
    if (block_len > PAGE_SIZE512)
        block_len = PAGE_SIZE512;
...
static s32 __card_readcsd(s32 drv_no) {
    ...
    ret = __check_response(drv_no, _ioResponse[drv_no][0]);
    if (ret == 0) {
        if ((ret = __card_dataread(drv_no, g_CSD[drv_no], 16)) != 0)
            return ret;
    }
    return ret;
}
...
static s32 __card_readcid(s32 drv_no) {
    ...
    ret = __check_response(drv_no, _ioResponse[drv_no][0]);
    if (ret == 0) {
        if ((ret = __card_dataread(drv_no, g_CID[drv_no], 16)) != 0)
            return ret;
    }
    return ret;
}
...
static s32 __card_sd_status(s32 drv_no) {
    ...
    if (_ioPageSize[drv_no] != 64) {
        ...
        if ((ret = __card_setblocklen(drv_no, _ioPageSize[drv_no])) != 0)
            return ret;
    }
    if ((ret = __card_sendappcmd(drv_no)) != 0)
        return ret;
    if ((ret = __card_sendcmd(drv_no, 0x0d, NULL)) != 0)
        return ret;
    if ((ret = __card_response2(drv_no)) != 0)
        return ret;
...
static bool __card_check(s32 drv_no) {
    ...
    if (drv_no == 0 && _ioCardSelect[drv_no] != EXI_DEVICE_0) {
        if (EXIGetID(drv_no, _ioCardSelect[drv_no], &id) == 0)
            return false;
        if (id != 0xffffffff)
            return false;
        return true;
    }
    while ((ret = EXIProbeEx(drv_no)) == 0)
        ;
    if (ret != 1)
        return false;

    if (EXIGetID(drv_no, EXI_DEVICE_0, &id) == 0)
        return false;
    ...
    if (drv_no != 2 && _ioCardSelect[drv_no] == EXI_DEVICE_0) {
        if (!(EXIGetState(drv_no) & EXI_STATE_ATTACHED)) {
            if (EXIAttach(drv_no, __card_exthandler) == 0)
                return false;
#ifdef _CARDIO_DEBUG
...
void sdgecko_initIODefault(void) {
    ...
    for (i = 0; i < MAX_DRIVE; ++i) {
        ...
        _ioCardFreq[i] = EXI_FREQ_32M;
        OSInitThreadQueue(&_ioEXILock[i]);
    }
}
...
s32 sdgecko_initIO(s32 drv_no) {
    ...
    if (_ioRetryCnt >= 3)
        _ioCardFreq[drv_no] = EXI_FREQ_16M;
    if (_ioRetryCnt >= 6) {
        _ioRetryCnt = 0;
        _ioCardFreq[drv_no] = EXI_FREQ_32M;
        return CARDIO_ERROR_IOERROR;
    }
    ...
    if (_ioCardInserted[drv_no] == true) {
        ...
        if (__card_sendCMD8(drv_no) != 0)
            goto exit;
#ifdef _CARDIO_DEBUG
        printf("Response %02X,%02X,%02X,%02X,%02X\n", _ioResponse[drv_no][0], _ioResponse[drv_no][1],
               _ioResponse[drv_no][2], _ioResponse[drv_no][3], _ioResponse[drv_no][4]);
#endif
        if (!(_ioResponse[drv_no][0] & MMC_ERROR_IDLE))
            goto exit;
        else if (_ioResponse[drv_no][0] == MMC_ERROR_IDLE) {
            if ((_ioResponse[drv_no][3] == 1) && (_ioResponse[drv_no][4] == 0xAA))
                _initType[drv_no] = TYPE_SDHC;
            else
                goto exit;
        }

        if (__card_sendopcond(drv_no) != 0)
            goto exit;

        if (_initType[drv_no] == TYPE_SDHC) {
            if (__card_sendCMD58(drv_no) != 0)
                goto exit;
#ifdef _CARDIO_DEBUG
            printf("Response %02X,%02X,%02X,%02X,%02X\n", _ioResponse[drv_no][0], _ioResponse[drv_no][1],
                   _ioResponse[drv_no][2], _ioResponse[drv_no][3], _ioResponse[drv_no][4]);
            ...
            if (_ioResponse[drv_no][1] & 0x40)
                _ioAddressingType[drv_no] = CARDIO_ADDRESSING_BLOCK;
        }

        _ioPageSize[drv_no] = PAGE_SIZE512;
        if (__card_setblocklen(drv_no, _ioPageSize[drv_no]) != 0)
            goto exit;

        if (__card_sd_status(drv_no) != 0)
            goto exit;
...
s32 sdgecko_readSector(s32 drv_no, void* buf, u32 sector_no) {
    ...
    if ((ret = __card_sendcmd(drv_no, 0x11, arg)) != 0)
        return ret;
...
s32 sdgecko_readSectors(s32 drv_no, u32 sector_no, u32 num_sectors, void* buf) {
    ...
    if ((ret = __card_sendcmd(drv_no, 0x12, arg)) != 0)
        return ret;
    if ((ret = __card_response1(drv_no)) != 0)
        return ret;
    ...
    if ((ret = __card_sendcmd(drv_no, 0x0C, NULL)) != 0)
        return ret;
...
s32 sdgecko_writeSector(s32 drv_no, const void* buf, u32 sector_no) {
    ...
    if ((ret = __card_sendcmd(drv_no, 0x18, arg)) != 0)
        return ret;
    if ((ret = __card_response1(drv_no)) != 0)
        return ret;
    if ((ret = __card_datawrite(drv_no, ptr, _ioPageSize[drv_no])) != 0)
        return ret;
...
s32 sdgecko_writeSectors(s32 drv_no, u32 sector_no, u32 num_sectors, const void* buf) {
    ...
    if ((ret = __card_sendappcmd(drv_no)) != 0)
        return ret;
    if ((ret = __card_sendcmd(drv_no, 0x17, arg)) != 0)
        return ret;
    if ((ret = __card_response1(drv_no)) != 0)
        return ret;
    ...
    if ((ret = __card_sendcmd(drv_no, 0x19, arg)) != 0)
        return ret;
    if ((ret = __card_response1(drv_no)) != 0)
        return ret;

    for (i = 0; i < num_sectors; i++) {
        if ((ret = __card_multidatawrite(drv_no, ptr, _ioPageSize[drv_no])) != 0)
            return ret;
        if ((ret = __card_dataresponse(drv_no)) != 0) {
            if ((ret = __card_sendcmd(drv_no, 0x0C, arg)) != 0)
                return ret;
            return __card_stopresponse(drv_no);
        }
        ptr += _ioPageSize[drv_no];
    ...
    if ((ret = __card_multiwritestop(drv_no)) != 0)
        return ret;
    if ((ret = __card_sendcmd(drv_no, 0x0D, NULL)) != 0)
        return ret;
    return __card_response2(drv_no);
...
bool sdgecko_isSDHC(s32 drv_no) {
    return (_initType[drv_no] == TYPE_SDHC);
}
