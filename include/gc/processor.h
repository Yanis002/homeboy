...
#define STACK_ALIGN(type, name, cnt, alignment)                                                                      \
    u8 _al__##name[(                                                                                                 \
        ...
        (((sizeof(type) * (cnt)) % (alignment)) > 0 ? ((alignment) - ((sizeof(type) * (cnt)) % (alignment))) : 0))]; \
...
#define mfpvr()                                         \
    ({                                                  \
        register u32 _rval;                             \
        __asm__ __volatile__("mfpvr %0" : "=r"(_rval)); \
        _rval;                                          \
    })
...
#define mfdcr(_rn)                                                        \
    ({                                                                    \
        register u32 _rval;                                               \
        __asm__ __volatile__("mfdcr %0," __stringify(_rn) : "=r"(_rval)); \
        _rval;                                                            \
    })
...
#define mfmsr()                                         \
    ({                                                  \
        register u32 _rval;                             \
        __asm__ __volatile__("mfmsr %0" : "=r"(_rval)); \
        _rval;                                          \
    })
...
#define mfdec()                                         \
    ({                                                  \
        register u32 _rval;                             \
        __asm__ __volatile__("mfdec %0" : "=r"(_rval)); \
        _rval;                                          \
    })
...
#define cntlzw(_val)                                                         \
    ({                                                                       \
        register u32 _rval;                                                  \
        __asm__ __volatile__("cntlzw %0, %1" : "=r"((_rval)) : "r"((_val))); \
        _rval;                                                               \
    })
...
#define _CPU_FPR_Enable()                                                                       \
    {                                                                                           \
        register u32 _val = 0;                                                                  \
        __asm__ __volatile__("mfmsr %0; ori %0,%0,0x2000; mtmsr %0" : "=&r"(_val) : "0"(_val)); \
    }

#define _CPU_FPR_Disable()                                                                          \
    {                                                                                               \
        register u32 _val = 0;                                                                      \
        __asm__ __volatile__("mfmsr %0; rlwinm %0,%0,0,19,17; mtmsr %0" : "=&r"(_val) : "0"(_val)); \
    }
